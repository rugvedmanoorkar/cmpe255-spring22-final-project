    ...
    if request.method == "POST":
        ...
        if request.files:
            ...
            if allowed_image(image.filename):
                ...
                result = model(os.path.join(app.config["IMAGE_UPLOADS"], filename),svm_path,"SVM")
                ...
                result = model(os.path.join(app.config["IMAGE_UPLOADS"], filename),rf_path,"RF")
                print("Result ", result[0])
...
def image_convert2(path,fs,model_type):
    images = []
    labels = []
    normal_limit=1
    p_limit=0
...
def model(path, model_path,model_type):
    print("PATH ", path)
    fs = np.empty((1,0))
    fs2 = image_convert2(path,fs,model_type)
